...
def get_planner_system_prompt(tool_schemas: List[Dict[str, Any]], known_facts: str = "") -> str:
    ...
    return (
        ...
        "5d. Decisions between options REQUIRE 'finance_calc' option evaluators: 'prepay_vs_invest' (prepay a loan or invest spare money), 'rent_vs_buy' (rent or buy a home), 'loan_tenure_choice' (shorter vs longer loan tenure), 'financial_health_check' (overall check-up / what should I do with my money / where do I stand). Use the user's numbers and Known User Facts; leave optional assumptions (returns, growth) out unless the user gave them.\n"
        ...
        "5c. Indian income-tax computations (tax payable, regime comparison, effect of deductions, capital-gains tax) REQUIRE the 'tax_calc' tool if registered. Pass ANNUAL amounts (convert monthly salary x 12). Do not use 'research' for current slab rates — 'tax_calc' already contains them. If the user names a tax/financial/assessment year (e.g. '2019-20'), still call 'tax_calc' and pass it as 'tax_year' exactly — the tool itself reports unsupported years. Specialised tax_calc operations: 'hra_exemption' (monthly basic, HRA and rent), 'house_property_income', 'capital_gains_tax' (one sale with dates), 'advance_tax_plan', 'itr_form_choice' (which ITR form) and 'tax_saving_finder' (unused deductions / is my regime right). Pass HRA exemption and house-property results into compute_tax as 'hra_exempt' / 'house_property_income'.\n"
        "7b. Questions about a rule, limit, eligibility, deadline, legal section or borrower right (e.g., 'What is the 80C limit?', 'How is HRA exemption calculated?', 'Last date to file ITR?', 'Can a recovery agent call me at night?') REQUIRE 'rules_lookup' if registered — not web_search or research. Pass 'tax_year' only if the user named one.\n"
        "7c. Questions about a document the user uploaded are answered from its summary (no tool needed) unless a calculation is required — e.g. tax from Form 16 figures uses 'tax_calc'.\n"
